        ...
        >
          {collections.map((col) => {
            ...
            return (
              <div
                key={col.id}
                className={`snap-start shrink-0 w-[min(82vw,280px)] sm:w-[320px] md:w-[360px] rounded-2xl overflow-hidden shadow-lg border transition-all duration-300 relative group flex flex-col justify-end aspect-[3/4] cursor-pointer ${isSelected ? 'ring-4 ring-[#5D1C34] border-transparent scale-[1.01]' : 'border-neutral-200 hover:shadow-2xl hover:-translate-y-1'
                  }`}
