...
export function SelectedProductCard({ product, onReplace }: { product: Product; onReplace: () => void }) {
  return (
    <div className="flex items-center gap-4 p-4 bg-white rounded-xl border border-[#CDBCAB]">
      ...
      <button onClick={onReplace} type="button" className="shrink-0 text-label-sm font-semibold text-[#38140C] hover:text-[#38140C]/80 transition-colors flex items-center gap-1 whitespace-nowrap bg-transparent border-0 cursor-pointer">
...
export function CatalogModal({ isOpen, onClose, onSelectProduct, products, currentProductId, initialCategory = 'ALL' }: CatalogModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="z-[120] max-w-[calc(100%-2rem)] sm:max-w-[620px] max-h-[84vh] overflow-hidden rounded-2xl border border-[#CDBCAB] bg-[#FDFAF7] p-0 shadow-xl">
        <div className="px-6 py-4 border-b border-[#CDBCAB]">
          <div>
            <DialogTitle className="text-[20px] font-semibold text-[#38140C]">Chọn trang phục thử đồ</DialogTitle>
        ...
        <div className="p-4 border-b border-[#CDBCAB] bg-[#EFE9E1] flex flex-col gap-3">
          <input type="text" placeholder="Tìm kiếm sản phẩm, danh mục..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full h-10 px-4 rounded-xl border border-[#CDBCAB] bg-white text-body-sm focus:outline-none focus:border-[#38140C] focus:ring-2 focus:ring-[#38140C]/20 transition-all" />
          <div className="flex gap-1.5 overflow-x-auto no-scrollbar">
            {([
              ...
              <button key={cat.id} type="button" onClick={() => setSelectedCat(cat.id)} className={`px-3 py-1 rounded-lg text-[12px] font-semibold transition-all whitespace-nowrap ${selectedCat === cat.id ? 'bg-[#38140C] text-white shadow-2xs' : 'bg-white text-neutral-600 border border-[#CDBCAB] hover:bg-neutral-100'}`}>
                {cat.label}
              </button>
            ))}
          </div>
        ...
        <div className="max-h-[52vh] overflow-y-auto p-4 grid grid-cols-1 sm:grid-cols-2 gap-3 no-scrollbar">
          {filteredProducts.map((p) => {
            ...
            return (
              <button type="button" key={p.id} onClick={() => { onSelectProduct(p); onClose(); }} className={`relative flex gap-3 p-3 rounded-xl border cursor-pointer text-left transition-all ${isSelected ? 'border-[#38140C] bg-[#38140C]/5 ring-1 ring-[#38140C]' : 'border-[#CDBCAB] bg-white hover:border-[#38140C]/60 hover:bg-neutral-50'}`}>
                ...
                <div className="flex-col flex justify-between min-w-0 py-0.5">
                  <div>
                    <h4 className="text-[12px] font-bold text-[#38140C] line-clamp-1 leading-snug">{p.name}</h4>
                    <span className="text-[10px] text-neutral-400 font-semibold mt-0.5 block">{p.brand}</span>
                  </div>
                  <span className="text-[12px] font-bold text-[#38140C]">{p.price}</span>
                </div>
                {isSelected && <Check className="absolute right-3 top-3 h-4 w-4 text-[#38140C]" aria-label="Đang chọn" />}
              </button>
            );
          })}
