...
export function ChatMessage({ message, onRetry }: ChatMessageProps) {
  ...
  return (
    <div
      className={`group flex items-start gap-3 w-full animate-in fade-in slide-in-from-bottom-2 duration-300 ${isUser ? 'justify-end' : 'justify-start'
        }`}
    >
      {/* Assistant Avatar */}
      {!isUser && (
      ...
      <div className={`flex flex-col gap-1.5 max-w-[85%] sm:max-w-[78%] md:max-w-[70%] ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        {/* Message Bubble */}
        <div
          className={`relative px-4 py-3.5 text-body-sm leading-relaxed transition-all ${isUser
