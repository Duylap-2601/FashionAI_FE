...
function getCategoryGroup(product: Product): 'Áo' | 'Quần & Váy' | 'Suit đầy đủ' {
  ...
  if (cat.includes('suit') || cat.includes('toan than') || cat.includes('full_body') || name.includes('suit') || name.includes('nguyên bộ')) {
    return 'Suit đầy đủ';
  }
  if (cat.includes('quan') || cat.includes('vay') || cat.includes('lower') || name.includes('quần') || name.includes('váy')) {
    return 'Quần & Váy';
  }
  return 'Áo';
}
...
export default function ProductListing() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Price range filter
    result = result.filter(p => p.numericPrice <= priceRange);
    ...
    return result;
  }, [allProducts, searchQuery, activeTab, selectedSubCategories, selectedSizes, selectedColors, priceRange, sortBy]);
  ...
  const activeChips = useMemo(() => {
    ...
    if (priceRange < maxPriceLimit) {
      chips.push({
        id: 'price',
        label: `≤ ${(priceRange / 1000).toLocaleString('vi-VN')}kđ`,
        onRemove: () => setPriceRange(maxPriceLimit)
      });
    }
    ...
    return chips;
  }, [activeTab, searchQuery, priceRange, maxPriceLimit, selectedSizes, selectedColors, selectedSubCategories]);
              ...
              <StaggerContainer className={`grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-${isSidebarOpen ? '5' : '6'} gap-2 md:gap-3 mb-12 transition-all duration-300`}>
                {paginatedProducts.map(product => (
                  <StaggerItem key={product.id} className="group">
                    <Link href={`/products/${product.id}`} className="group flex flex-col bg-white border border-neutral-100 rounded-lg overflow-hidden hover:shadow-md transition-shadow h-full">
                      <div className="relative aspect-[3/4] bg-neutral-100 overflow-hidden">
                        ...
                        <button
                            ...
                            toast.custom((t) => (
                              <div className="bg-[#FDFBF7] border-l-4 border-[#5D1C34] border-y border-r border-[#E5DFD5] p-4 rounded-xl shadow-lg flex items-start gap-3.5 max-w-[380px] w-full relative">
                                ...
                                <div className="flex-1 min-w-0 pr-4">
                                  ...
                                  <p className="text-[11px] text-neutral-500 mt-0.5">Màu: {color} | Size: M | SL: 1</p>
                                </div>
                          ...
                          className="absolute bottom-1.5 right-1.5 w-7 h-7 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center text-brand-navy shadow-sm opacity-0 translate-y-1 group-hover:opacity-100 group-hover:translate-y-0 transition-all"
                        >
